...
class BaseBackend(BaseEvaluableObject):
    ...
    def _get_state(self, iter_num: int) -> Dict[str, Any]:
        ...
        inter_param=self.graph.get_parameters_as_tensor(filter_requires_grad=FILTER_OFF)
        param = inter_param.cpu() if inter_param is not None else inter_param
...
class IntegerBackend(BaseBackend):
    ...
    def _continuous_param_config(self) -> ParamConfig:
        kwargs_continuous_, _ = self._split_kwargs()
        kwargs_continuous=copy.deepcopy(kwargs_continuous_)
        del kwargs_continuous["filter_requires_grad"]
        return self.graph.get_parameters_as_config(**kwargs_continuous) if kwargs_continuous else {}
...
class GridSearch(HybridBackend):
    ...
    def evaluate(self, num_iters: int = 3, start_iter: int = 0,
        ...
        '''# test loss
        losses = loss_func(graph.evaluate()).detach().item()
        pass'''
        ...
        continuous_info=self._continuous_param_config()
        inter=[]
        for items in continuous_info.values():
            for k,v in items.items():
                for i in range(v['value'].numel()):
                    if isinstance(v["scale"][0] , torch.Tensor):
                        inter.append((v["scale"][0][i].item(),v["scale"][1][i].item()))
                    else:
                        inter.append(v["scale"])

        continuous_range=[(i[0],i[1])  for i  in  inter]
        ...
        for it in range(num_iters):
            ...
            if params is not  None :
                for i in range(len(params)):
                    vl,vh=continuous_range[i]
                    val_range = np.linspace(0.0, 1.0, search_res + 1).tolist()
