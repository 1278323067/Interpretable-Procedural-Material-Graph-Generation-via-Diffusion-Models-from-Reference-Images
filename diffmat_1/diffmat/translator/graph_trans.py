...
class MaterialGraphTranslator(BaseGraphTranslator[MaterialNodeTranslator]):
    ...
    def __init__(self, root: Union[PathLike, ET.Element], res: int, external_noise: bool = True,
                 ...
                 integer_option: str = 'integer',paths : List = None ):
        ...
        #self.external_inputs=self.gen_bitmap(paths) if paths is not  None else {}
    def gen_bitmap(self,paths: List):
        imgs={}
        for i,path in enumerate(paths) :
            img_np = np.array( Image.open(Path(path)) )
            ...
            if img_np.ndim < 3:
                ...
                intern_img = th.from_numpy(img_np).unsqueeze(0).to("cuda")
            else:
                intern_img =  th.from_numpy(img_np).movedim(2, 0).to("cuda")
            imgs.update({f"bitmap_{i}" : intern_img.unsqueeze(0)})
        return imgs
    ...
    def translate(self, seed: int = -1, use_alpha: bool = True, normal_format: str = 'dx',
                  gen_external_input: bool = False, external_input_folder: PathLike = '.',
                  device: DeviceType = 'cpu' , external_input_path : str = None ) -> MaterialGraph:
        ...
        if gen_external_input:
            ...
            external_inputs = self.gen_bitmap(external_input_path) if external_input_path is not  None else {}
